...
grep -q "\['filament_spool_id'\]\[channel\]" "$F" || sed -i "s/self.print_task_config\['filament_sku'\]\[channel\] = info\['SKU'\]/self.print_task_config['filament_sku'][channel] = info['SKU']\n        self.print_task_config['filament_spool_id'][channel] = info['SPOOL_ID']/" "$F"
...
grep -q "\['filament_spool_id'\]\[config_extruder\]" "$F" || sed -i "s/self.print_task_config\['filament_sku'\]\[config_extruder\] = 0/self.print_task_config['filament_sku'][config_extruder] = 0\n            self.print_task_config['filament_spool_id'][config_extruder] = filament_spool_id/" "$F"
